...
class _AddKeepNoteViewState extends State<AddKeepNoteView>
      ...
      _textControllers[id]!.addListener(() {
        ...
        if (parts.length >= 2) {
          ...
          if (itemIndex != null) {
            ...
            if (blockIdx != -1 &&
                _ctrl.blocks[blockIdx].type == KeepNoteType.checklist) {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final isImageBg =
          ...
          _ctrl.selectedBackground.value! <
              KeepController.backgroundImages.length;
      final customTextColor = _ctrl.getTextColor(_ctrl.selectedTextColor.value);
      final textColor =
          customTextColor ??
          (isImageBg ? Colors.white : _contrastColor(bgColor));
      ...
      return PopScope(
        ...
        onPopInvokedWithResult: (didPop, _) async {
          ...
          if (_isViewOnly) {
            ...
            if (widget.viewOnly) {
              await _ctrl.saveNote(
                existing: widget.existingNote,
                popAfterSave: false,
              );
              ...
              Positioned.fill(
                child: Obx(
                  () => ImageFiltered(
                    ...
                    child: Image.asset(
                      KeepController.backgroundImages[_ctrl
                          .selectedBackground
                          .value!],
                      fit: BoxFit.cover,
                    ),
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Obx(
                                    () => Column(
                                      children: _ctrl.blocks
                                          .asMap()
                                          .entries
                                          .map((entry) {
                                            return _buildBlockWrapper(
                                              entry.key,
                                              entry.value,
                                              textColor,
                                            );
  ...
  Widget _buildTextBlock(KeepBlock block, Color textColor) {
    final controller = _getQuillController(
      block.id,
                      ...
                      Row(
                        children: [
                          Icon(
                            ...
                            color: uiColor.withValues(alpha: 0.5),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Slider(
                              ...
                              onChanged: (val) {
                                ...
                                if (_activeQuillController != null) {
                                  _activeQuillController!.skipRequestKeyboard =
                                      true;
                                  final attr = quill.Attribute(
                                    'size',
  ...
  Widget _buildAlignButton(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? uiColor.withValues(alpha: 0.15)
              : Colors.transparent,
          borderRadius: BorderRadius.circular(12),
        ),
        child: Icon(
          icon,
          color: isSelected ? uiColor : uiColor.withValues(alpha: 0.5),
        ),
      ),
    );
  }
  ...
  Widget _buildStyleButton(
    ...
    final c = _activeQuillController;
    final isSelected =
        c?.getSelectionStyle().containsKey(attribute.key) ?? false;
    return GestureDetector(
  ...
  Widget _buildColorButton(
    ...
    if (c != null) {
      ...
      if (colorAttr != null && colorAttr.value != null) {
        ...
        isSelected =
            attrValue == searchHex ||
            attrValue == '#ff${searchHex.substring(1)}';
      }
    }
                          ...
                          child: Obx(() {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                shape: BoxShape.circle,
                                border: isSelected
                                    ? Border.all(color: uiColor, width: 2)
                                    : null,
                              ),
                              child: isSelected
                                  ? Icon(Icons.check, color: _contrastColor(c))
                                  : null,
                            );
                          }),
                        );
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                image: DecorationImage(
                                  image: AssetImage(
                                    KeepController.backgroundImages[pIndex],
                                  ),
                                  fit: BoxFit.cover,
                                ),
                              ),
  ...
  Future<void> _showCustomDateTimePickerBottomSheet(
...
class _DrawingCanvasState extends State<_DrawingCanvas> {
    ...
    try {
      ...
      if (parts.length >= 3) {
        ...
        for (final line in lines) {
          ...
          pts.add(null);
          _strokes.add(
            _Stroke(points: pts, color: strokeColor, width: strokeWidth),
          );
        }
      }
    } catch (e) {
  ...
  String _serialize() {
    ...
    for (final stroke in _strokes) {
      ...
      if (pts.isNotEmpty) {
        lines.add(
          '${stroke.color.toARGB32()};${stroke.width.toStringAsFixed(1)};${pts.join(',')}',
        );
      }
    }
        ...
        if (!widget.viewOnly) ...[
          ...
          GestureDetector(
            ...
            onVerticalDragUpdate: (d) {
              setState(() {
                _canvasHeight = (_canvasHeight + d.delta.dy).clamp(
                  _minHeight,
                  _maxHeight,
                );
              });
    ...
    return Column(
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            fontSize: 13,
            color: isRecording ? Colors.redAccent : tc.withValues(alpha: 0.45),
            fontWeight: FontWeight.w600,
          ),
        ),
